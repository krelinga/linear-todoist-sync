...
- **Errors carry context, never bare messages.** Neither API names the subject of a failure — Linear answers a lookup for a deleted issue with `Entity not found: Issue - Could not find referenced Issue`, which identifies neither the issue nor the Todoist project whose description pointed at it, and a poll cycle walks many of both. So `src/errors.ts` provides `ContextualError` / `withContext` / `errorFields`: the client layer wraps every API call with its operation name and arguments, callers add whatever they know that the client cannot, and the layer that logs spreads `errorFields(err)` to get the whole chain as structured fields. Do not write `error: err instanceof Error ? err.message : String(err)` in a new log call — that is the pattern this replaced. `extractHttpStatus`/`httpRetryClassifier` in `src/retry.ts` walk the `cause` chain precisely so wrapping stays invisible to retry classification and to `getIssueRaw`'s "a 4xx means the issue is gone" check.
